...
df <- data.frame(ISL$sentiment_index, ISV[c('sentiment_index', 'selic')],
                 ibcbr$ibc_br_adj, hiato$cycle, ipca$ipca)

colnames(df) <- c('ISL', 'ISV', 'selic', 'ibcbr', 'hiato', 'ipca')

rm(ISV, ISL, ibcbr, ipca, selic, hiato)
...
diff_data <- data.frame(d_isv, d_isl, d_ibcbr, d_ipca, d_selic, d_hiato)
